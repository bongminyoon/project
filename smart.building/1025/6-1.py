...
def run(models, stranger_dir):
    ...
    detection_count = 0  # 얼굴 연속 감지 카운터
    confidence_suc = 0
    confidence_fai = 0
    confidence_cnt = 0
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            if camera_active:
                ...
                if face is not None:
                    detection_count += 1  # 얼굴이 감지되면 카운터 증가
                else:
                    detection_count = 0  # 얼굴이 감지되지 않으면 카운터 초기화

                # 5회 연속 얼굴 감지 시 얼굴 인식 코드로 진입
                if detection_count >= 5:
                    try:
                        ...
                        detection_count = 0  # 인식 후 연속 감지 카운터 초기화
