...
def take_pictures_dnn(name):
    ...
    try:
        while count < total_pictures:  # 최대 500장까지 저장
            ...
            no_face_count = 0  # 얼굴이 검출되지 않는 경우를 세기 위한 변수
            ...
            while current_round_pictures < pictures_per_round:
                ...
                if face is not None:
                    ...
                    # 현재 시간으로 파일 이름을 생성하여 저장
                    timestamp = time.strftime("%Y%m%d-%H%M%S")
                    count += 1
                    current_round_pictures += 1
                    file_name_path = user_dir + f'user_{timestamp}_{count}.jpg'
                ...
                # 얼굴을 10번 연속 찾지 못했을 경우 중단
                if no_face_count > 10:
                    print("얼굴을 찾지 못한 횟수가 10번을 넘었습니다. 종료합니다.")
                    break
