...
# 얼굴 인식 실행 함수
def run(models, stranger_dir, socket):
    ...
    # 소켓을 이용한 통신 시작 (소켓을 메인 함수에서 전달받음)
    socket.sendall(b'FR:room201')  # 초기 메시지 전송
    ...
    while True:
        ...
        try:
                ...
                if confidence_cnt < 21:
                    if confidence >= 85:
                        ...
                        if confidence_suc == 15:
                            print("얼굴 인식 성공")
                            socket.sendall(b'FR:room201:success')  # 성공 메시지 전송
                    else:
                        ...
                        if confidence_fai == 5:
                            ...
                            socket.sendall(f'FR:room201:failed:{failed_img_path}'.encode())  # 실패 메시지 전송
            ...
            if data == 'ROOM_201:failure capture':
                ...
                socket.sendall(f'FR:room201:failed:{capture_path}'.encode())  # 실패 이미지 저장 및 전송
...
if __name__ == "__main__":
    ...
    if len(models) == 0:
        ...
        # 메인 함수에서 소켓을 연결하고 `run` 함수에 전달
        HOST = '127.0.0.1'
        PORT = 9000
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            run(models, stranger_dir, s)  # 소켓을 run 함수에 전달
