...
# CUDA 가속을 사용한 얼굴 추출 함수
def face_extractor_dnn_gpu(img, net):
    h, w = img.shape[:2]
    faces = []
    face_box = None
    ...
    # DNN 얼굴 검출
    blob = cv2.dnn.blobFromImage(gpu_img, 1.0, (300, 300), (104.0, 177.0, 123.0))
    net.setInput(blob)
    ...
    return faces[0], face_box if len(faces) > 0 else None, None
        ...
        try:
            ...
            if data == 'FR:room_201:request_capture':
                ...
                if frame is not None:
                    ...
                    s.sendall(f'FR:room_201:capture:{capture_path}'.encode())
                else:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            if camera_active:
                ...
                face, face_box = face_extractor_dnn_gpu(frame, net)  # CUDA 가속 사용
                        ...
                        if confidence_cnt < 21:
                            ...
                            if confidence_cnt == 20:
                                if confidence_suc >= 15:
                                    ...
                                    s.sendall(f'FR:room_201:failure:{failed_img_path}'.encode())
                                    time.sleep(10)
            ...
            if cv2.waitKey(1) == 13:  # Enter 키를 눌러 종료
                break
